...
class VehicleSlot extends StatefulWidget {
  final String areaName; // Pass the selected area name to this page

  const VehicleSlot({super.key, required this.areaName});

  @override
  _VehicleSlotState createState() => _VehicleSlotState();
}

class _VehicleSlotState extends State<VehicleSlot> {
  ...
  Future<void> _updateSlotStatus(String slotKey, String status) async {
    try {
      final areaRef =
          _firestore.collection('vehicle_area').doc(widget.areaName);
